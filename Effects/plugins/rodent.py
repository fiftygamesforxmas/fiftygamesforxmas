...
def _noise_gate(x: np.ndarray) -> np.ndarray:
    """Attenuate junk below the guitar floor without changing playing dynamics much."""
    env = np.maximum(np.abs(x), 1e-8)
    gain = np.clip(env / GATE_THRESH, GATE_FLOOR, 1.0)
    return x * gain
...
def _process_mono(
    ...
    # Fixed pad only — do NOT peak-normalize live buffers.
    x = x * input_level
    x = _noise_gate(x)
    ...
    x = _apply_sos(x, _design_sos_lpf(8000.0, osr, 1))

    bass = _apply_sos(x, _design_sos_hpf(60.0, osr, 1))
    mids = _apply_sos(x, _design_sos_hpf(1500.0, osr, 1))

    # Extra internal drive so a quieter input still hits the clipper
    g_lin = 8.0 + distortion * 420.0
    pre = x * 0.35 + bass * 0.45 + mids * 0.85
    pre *= g_lin

    hf_cut = 14000.0 - distortion * 8000.0
...
class RodentDistortion:
    ...
    def apply_effect(self, audio, sample_rate: int):
        ...
        input_level = float(np.clip(getattr(self, "input", 0.08), 0.02, 0.25))
